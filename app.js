...
function fillup(){
   ...
   for(i=0; i < key.length; i++){
      MobileBrand.innerHTML += `<option>${key[i]}</option>`
...
function fillupnext(){
   ...
   let nestedkeys = Object.keys(mobiles[MobileBrand.value])
...
function textnow(){
   text.innerHTML = `<div id="text">${ Object.values(mobiles [MobileBrand.value] [MobileModel.value])}</div>`
